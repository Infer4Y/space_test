package xyz.ignite4inferneo.space_test.client.renderer;

import java.awt.image.BufferedImage;
import java.util.Random;

/**
 * Utility for generating procedural block textures
 */
public class TextureUtils {
    private static final Random rand = new Random();

    public static BufferedImage createBlockTexture(String type, int size) {
        BufferedImage img = new BufferedImage(size, size, BufferedImage.TYPE_INT_ARGB);

        for (int y = 0; y < size; y++) {
            for (int x = 0; x < size; x++) {
                int color = switch(type.toLowerCase()) {
                    case "grass" -> generateGrassTopPixel(x, y, size);
                    case "grass_side" -> generateGrassSidePixel(x, y, size);
                    case "dirt" -> generateDirtPixel(x, y, size);
                    case "stone" -> generateStonePixel(x, y, size);
                    case "wood" -> generateWoodPixel(x, y, size);
                    case "leaves" -> generateLeavesPixel(x, y, size);
                    default -> 0xFFFFFF;
                };
                img.setRGB(x, y, 0xFF000000 | color);
            }
        }
        return img;
    }

    private static int generateGrassTopPixel(int x, int y, int size) {
        int baseR = 30;
        int baseG = 140;
        int baseB = 40;

        double noise = perlinNoise(x / 4.0, y / 4.0) * 0.5
                + perlinNoise(x / 2.0, y / 2.0) * 0.3
                + perlinNoise(x, y) * 0.2;

        int r = baseR + (int)(noise * 40);
        int g = baseG + (int)(noise * 60);
        int b = baseB + (int)(noise * 30);

        if ((x + y * 3) % 5 < 2 && rand.nextFloat() < 0.3f) {
            r += 20;
            g += 30;
            b += 15;
        }

        if (rand.nextFloat() < 0.05f) {
            r = Math.min(255, r + 40);
            g = Math.min(255, g + 50);
            b = Math.min(255, b + 20);
        }

        return clampRGB(r, g, b);
    }

    private static int generateGrassSidePixel(int x, int y, int size) {
        // Top third is grass, rest is dirt
        if (y < size / 3) {
            return generateGrassTopPixel(x, y, size);
        } else {
            return generateDirtPixel(x, y - size / 3, size);
        }
    }

    private static int generateDirtPixel(int x, int y, int size) {
        int baseR = 110;
        int baseG = 70;
        int baseB = 40;

        double noise = perlinNoise(x / 3.0, y / 3.0) * 0.6
                + perlinNoise(x, y) * 0.4;

        int r = baseR + (int)(noise * 50);
        int g = baseG + (int)(noise * 40);
        int b = baseB + (int)(noise * 30);

        if (rand.nextFloat() < 0.15f) {
            int pebble = rand.nextInt(40) - 20;
            r += pebble;
            g += pebble / 2;
            b += pebble / 3;
        }

        if (((x / 2) ^ (y / 2)) % 3 == 0) {
            r = Math.max(0, r - 20);
            g = Math.max(0, g - 15);
            b = Math.max(0, b - 10);
        }

        return clampRGB(r, g, b);
    }

    private static int generateStonePixel(int x, int y, int size) {
        int baseGray = 100;

        double noise = perlinNoise(x / 5.0, y / 5.0) * 0.5
                + perlinNoise(x / 2.0, y / 2.0) * 0.3
                + perlinNoise(x, y) * 0.2;

        int gray = baseGray + (int)(noise * 60);

        int r = gray;
        int g = Math.max(0, gray - 5);
        int b = Math.max(0, gray - 8);

        return clampRGB(r, g, b);
    }

    private static int generateWoodPixel(int x, int y, int size) {
        int baseR = 120;
        int baseG = 80;
        int baseB = 40;

        // Vertical grain
        double grainNoise = perlinNoise(x / 6.0, y / 2.0);

        int r = baseR + (int)(grainNoise * 40);
        int g = baseG + (int)(grainNoise * 30);
        int b = baseB + (int)(grainNoise * 20);

        // Horizontal rings
        if (y % 4 < 2) {
            r -= 10;
            g -= 8;
            b -= 5;
        }

        return clampRGB(r, g, b);
    }

    private static int generateLeavesPixel(int x, int y, int size) {
        int baseR = 20;
        int baseG = 100;
        int baseB = 20;

        double noise = perlinNoise(x / 3.0, y / 3.0) * 0.7
                + perlinNoise(x, y) * 0.3;

        int r = baseR + (int)(noise * 50);
        int g = baseG + (int)(noise * 80);
        int b = baseB + (int)(noise * 50);

        // Random bright spots (light through leaves)
        if (rand.nextFloat() < 0.1f) {
            r = Math.min(255, r + 60);
            g = Math.min(255, g + 100);
            b = Math.min(255, b + 60);
        }

        // Darker spots for depth
        if ((x + y) % 5 < 2 && rand.nextFloat() < 0.4f) {
            r = Math.max(0, r - 30);
            g = Math.max(0, g - 40);
            b = Math.max(0, b - 30);
        }

        return clampRGB(r, g, b);
    }

    public static double perlinNoise(double x, double y) {
        int xi = (int)Math.floor(x);
        int yi = (int)Math.floor(y);

        double xf = x - xi;
        double yf = y - yi;

        double n00 = randomGradient(xi, yi, xf, yf);
        double n10 = randomGradient(xi + 1, yi, xf - 1, yf);
        double n01 = randomGradient(xi, yi + 1, xf, yf - 1);
        double n11 = randomGradient(xi + 1, yi + 1, xf - 1, yf - 1);

        double u = smoothstep(xf);
        double v = smoothstep(yf);

        double nx0 = lerp(n00, n10, u);
        double nx1 = lerp(n01, n11, u);

        return lerp(nx0, nx1, v);
    }

    private static double randomGradient(int ix, int iy, double x, double y) {
        int hash = (ix * 374761393 + iy * 668265263) & 0x7FFFFFFF;
        double angle = (hash % 628318) / 100000.0;
        return x * Math.cos(angle) + y * Math.sin(angle);
    }

    private static double smoothstep(double t) {
        return t * t * (3.0 - 2.0 * t);
    }

    private static double lerp(double a, double b, double t) {
        return a + t * (b - a);
    }

    private static int clampRGB(int r, int g, int b) {
        r = Math.max(0, Math.min(255, r));
        g = Math.max(0, Math.min(255, g));
        b = Math.max(0, Math.min(255, b));
        return (r << 16) | (g << 8) | b;
    }
}